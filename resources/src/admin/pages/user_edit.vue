        ...
        <vue-form :state="formstate" @submit.prevent="onSubmit">
            <div class="card bg-primary-card">
                <h4 class="card-header">
                    <div><i class="fas fa-user-edit"></i> Edit User Account</div>
                </h4>
                <div class="card-body">
                    <div class="tabs">
                        <div>
                            <ul role="tablist" class="nav nav-tabs" id="user_edit_tab_controls">
                        ...
                        <div class="tab-content" id="user_edit_tab_content">
                            <div class="tab-pane fade show active" id="base-info" role="tabpanel" aria-labelledby="base-info-tab">
                                <div class="row mt-5">
                                        ...
                                        <div role="group" class="form-group form-inline">
                                            ...
                                            <validate tag="div">
                                                <input type="email" id="email" name="email" v-model="model.email" class="form-control"
                                                       pattern="[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,30}$" required disabled/>
                                    ...
                                    <div class="col-md-6">
                                        <div role="group" class="form-group form-inline">
                                            <label for="plan" class="d-block">Plan*</label>
                                            <validate tag="div">
                                                <multiselect v-model="model.plan" :options="plans" placeholder="~~ Select a plan ~~"
                                                             label="plan_name" id="plan" name="plan" track-by="id" required></multiselect>
                                                <field-messages name="plan" show="$invalid && $submitted" class="text-danger">
                                                    <div slot="required">Plan is a required</div>
                                                </field-messages>
                                            </validate>
                                        </div>
                                        <div role="group" class="form-group form-inline">
                                            ...
                                            <validate tag="div">
                                                <multiselect v-model="model.subscription" :options="subscription" placeholder="~~ Select a status ~~"
                                                             label="name" id="subscription" name="subscription" track-by="name" required></multiselect>
                                                <field-messages name="subscription" show="$invalid && $submitted" class="text-danger">
                                                    <div slot="required">Subscription is a required</div>
                                                </field-messages>
                                            </validate>
                                        </div>
                                        <div role="group" class="form-group form-inline">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getMember() {
                this.$http.get(`/admin/users/${this.$route.params.id}/edit`).then((r) => {
                    this.model.name = r.data.name;
                    this.model.email = r.data.email;
                    this.model.role = r.data.role;
                    this.model.company = r.data.company;
                    this.model.address = r.data.address;
                    this.model.city = r.data.city;
                    this.model.postal_code = r.data.postal_code;
                    this.model.state_code = r.data.state_code;
                    this.model.phone = r.data.phone;
                    this.model.country = r.data.country;
                    this.model.subscription = r.data.subscription;
                    this.model.plan = r.data.plan;
                }).catch((e) => {
                    this.errorHandle(e);
                });
            },
            onSubmit() {
                if (this.formstate.$invalid) {
                    ...
                    let data = this.model;
                    data.flag = 'update';
                    this.$http.put(`/admin/users/${this.$route.params.id}`, data).then((r) => {
                        this.$router.push('/');
                    }).catch((e) => {
                        this.errorHandle(e);
                    });
